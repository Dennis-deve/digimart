...
export async function validateReceiver(input: { receiver: string; channel: string }) {
  const response = await providerFetch(`${base()}/open/transact/validate`, { method: 'POST', headers: transferHeaders(), body: JSON.stringify({ type: 1, receiver: input.receiver, channel: input.channel, currency: 'GHS', accountnumber: process.env.MOOLRE_ACCOUNT_NUMBER }) });
...
export async function sendTransfer(input: { receiver: string; channel: string; amount: number; externalref: string }) {
  const response = await providerFetch(`${base()}/open/transact/transfer`, { method: 'POST', headers: transferHeaders(), body: JSON.stringify({ type: 1, channel: input.channel, currency: 'GHS', amount: input.amount.toFixed(2), receiver: input.receiver, externalref: input.externalref, accountnumber: process.env.MOOLRE_ACCOUNT_NUMBER }) });
