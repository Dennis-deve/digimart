...
export async function fulfillRefer2Bundle(input: { network: string; dataPlan: string; beneficiary: string }) {
  ...
  const response = await fetch(`${r2bBase()}/order`, { method: 'POST', headers: r2bHeaders(), body: JSON.stringify({ network, data_plan: input.dataPlan, beneficiary: input.beneficiary }) });
...
export async function getRefer2BundleStatus(reference: string) {
  ...
  const response = await fetch(`${r2bBase()}/order-status?reference=${encodeURIComponent(reference)}`, { headers: r2bHeaders() });
...
export async function getRefer2BundleBalance() {
  if (!refer2BundleConfigured()) throw new Error('Refer2Bundle is not configured.');
  const response = await fetch(`${r2bBase()}/balance`, { headers: r2bHeaders() });
...
export async function registerRefer2BundleAFA(input: { fullName: string; phone: string; idCardNumber: string; location: string; idempotencyKey: string }) {
  if (!refer2BundleConfigured()) return { mode: 'sandbox-not-configured' as const, status: 'pending' };
  const response = await fetch(`${r2bBase()}/afa/register`, { method: 'POST', headers: r2bHeaders(), body: JSON.stringify({ full_name: input.fullName, phone: input.phone, id_card_number: input.idCardNumber, location: input.location, idempotency_key: input.idempotencyKey }) });
...
export async function getRefer2BundleAFAStatus(reference: string) {
  if (!refer2BundleConfigured()) throw new Error('Refer2Bundle is not configured.');
  const response = await fetch(`${r2bBase()}/afa/status/${encodeURIComponent(reference)}`, { headers: r2bHeaders() });
...
export async function fulfillBundleShopGHAFA(input: { phone: string; fullName?: string; idCardNumber?: string; location?: string; callback: string }) {
  ...
  const response = await fetch(`${process.env.BUNDLESHOPGH_BASE_URL ?? 'https://backend.mycledanet.com'}/api/afa-registration`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-API-Key': process.env.BUNDLESHOPGH_API_KEY }, body: JSON.stringify(body) });
